...
export const profiles = pgTable(
  ...
  (table) => [
    uniqueIndex("profiles_username_pk").using(
      "btree",
...
export const quests = pgTable(
  ...
  (table) => [check("check_start_before_end", sql`start_time < end_time`)],
);

export const questers = pgTable(
  "questers",
  {
    questId: uuid("quest_id").notNull(),
    userId: uuid("user_id").notNull(),
...
export const buddies = pgTable(
  ...
  (table) => [
    primaryKey({
      columns: [table.senderUserId, table.receiverUserId],
      name: "buddies_pk",
    }),
    check(
      "users_are_different",
      sql`sender_user_id IS DISTINCT FROM receiver_user_id`,
    ),
  ],
);
